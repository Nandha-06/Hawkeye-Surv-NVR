...
# viable stack. Called by Hawkeye skill-runtime-manager during installation.
#
...
emit() { echo "$1"; }  # JSON to stdout for Hawkeye to parse
...
find_python() {
    ...
    if command -v conda &>/dev/null; then
        ...
        conda create -n hawkeye-yolo2026 python=3.11 -y >/dev/null 2>&1
        # shellcheck disable=SC1091
        eval "$(conda shell.bash hook 2>/dev/null)"
        conda activate hawkeye-yolo2026
        echo "python3"
        return 0
    fi
...
if env.framework_ok:
    params = json.loads(os.environ.get('HAWKEYE_SKILL_PARAMS', '{}'))
    size = params.get('model_size', 'nano')
    s = 's' if size == 'small' else ('m' if size == 'medium' else ('l' if size == 'large' else 'n'))
    mf = f'yolo26{s}.pt'
    mn = f'yolo26{s}'
    from ultralytics import YOLO
    model = YOLO(mf)
    result = env.export_model(model, mn)
    if result:
...
# ─── Step 5.5: Download Re-ID models (if not present) ────────────────────────
...
MFNET_URL="https://huggingface.co/deepghs/insightface/resolve/main/buffalo_s/w600k_mbf.onnx"
MFNET_FILE="$MODELS_DIR/mobilefacenet.onnx"

if [ ! -f "$SCRFD_FILE" ]; then
    log "Downloading SCRFD face detector (1.5 MB)..."
    emit '{"event": "progress", "stage": "models", "message": "Downloading SCRFD face detector (1.5 MB)..."}'
    "$VENV_DIR/bin/python" -c "
import requests, sys
r = requests.get(sys.argv[1], allow_redirects=True, stream=True)
r.raise_for_status()
with open(sys.argv[2], 'wb') as f:
    for chunk in r.iter_content(8192):
        f.write(chunk)
" "$SCRFD_URL" "$SCRFD_FILE" 2>&1 || log "WARNING: SCRFD download failed"
    [ -f "$SCRFD_FILE" ] && log "SCRFD model downloaded."
else
    log "SCRFD model already present."
fi

if [ ! -f "$MFNET_FILE" ]; then
    log "Downloading MobileFaceNet (5.2 MB)..."
    emit '{"event": "progress", "stage": "models", "message": "Downloading MobileFaceNet (5.2 MB)..."}'
    "$VENV_DIR/bin/python" -c "
import requests, sys
r = requests.get(sys.argv[1], allow_redirects=True, stream=True)
r.raise_for_status()
with open(sys.argv[2], 'wb') as f:
    for chunk in r.iter_content(8192):
        f.write(chunk)
" "$MFNET_URL" "$MFNET_FILE" 2>&1 || log "WARNING: MobileFaceNet download failed"
    [ -f "$MFNET_FILE" ] && log "MobileFaceNet model downloaded."
else
    log "MobileFaceNet model already present."
fi

emit '{"event": "progress", "stage": "models", "message": "Face recognition models ready"}'
