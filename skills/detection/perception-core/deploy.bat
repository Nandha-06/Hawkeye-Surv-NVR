...
REM viable stack. Called by Hawkeye skill-runtime-manager during installation.
REM
...
REM Intelligent Windows Launcher discovery for Astral/Custom Python tags (bypasses experimental 3.14)
if not defined PYTHON_CMD (
    py -c "import sys" >nul 2>&1
    if !errorlevel! equ 0 (
        for /f "tokens=*" %%A in ('py -c "import subprocess, re; res = subprocess.run(['py', '--list'], capture_output=True, text=True); tags = []; [tags.append((re.search(r'(3\.(9|10|11|12|13))', line).group(1), re.search(r'-V:([^\s]+)', line).group(1))) for line in res.stdout.splitlines() if re.search(r'-V:([^\s]+)', line) and re.search(r'(3\.(9|10|11|12|13))', line)]; tags.sort(reverse=True); print(tags[0][1] if tags else '')" 2^>nul') do (
            if not "%%A"=="" (
                set "PYTHON_CMD=py -V:%%A"
            )
        )
    )
)
...
if defined ENV_CONFIG_DIR (
    ...
    set "_TMPBACK=%TEMP%\_hawkeye_detect_backend.txt"
...
if "!BACKEND!" neq "cpu" (
    ...
    "%VPYTHON%" -c "import sys,os,json; params=json.loads(os.environ.get('HAWKEYE_SKILL_PARAMS','{}')); size=params.get('model_size','nano'); s='s' if size=='small' else ('m' if size=='medium' else ('l' if size=='large' else 'n')); mf=f'yolo26{s}.pt'; mn=f'yolo26{s}'; sys.path.insert(0, os.environ['ENV_CONFIG_DIR']); from env_config import HardwareEnv; env=HardwareEnv.detect(); from ultralytics import YOLO; model=YOLO(mf); result=env.export_model(model, mn); print(f'Optimized model exported: {result}' if result else 'Export skipped or failed')" 2>&1
...
REM ─── Step 5.5: Download Re-ID models (if not present) ───────────────────────

set "MODELS_DIR=%SKILL_DIR%\models"
if not exist "%MODELS_DIR%" mkdir "%MODELS_DIR%"

set "SCRFD_URL=https://huggingface.co/ykk648/face_lib/resolve/main/face_detect/scrfd_onnx/scrfd_500m_bnkps.onnx"
set "SCRFD_FILE=%MODELS_DIR%\scrfd_500m_bnkps.onnx"

set "MFNET_URL=https://huggingface.co/deepghs/insightface/resolve/main/buffalo_s/w600k_mbf.onnx"
set "MFNET_FILE=%MODELS_DIR%\mobilefacenet.onnx"

if not exist "%SCRFD_FILE%" (
    echo %LOG_PREFIX% Downloading SCRFD face detector ^(1.5 MB^)...>&2
    echo {"event": "progress", "stage": "models", "message": "Downloading SCRFD face detector (1.5 MB)..."}
    "%VPYTHON%" -c "import requests,sys; r=requests.get(sys.argv[1],allow_redirects=True,stream=True); r.raise_for_status(); f=open(sys.argv[2],'wb'); [f.write(c) for c in r.iter_content(8192)]; f.close()" "%SCRFD_URL%" "%SCRFD_FILE%" 2>&1
    if !errorlevel! neq 0 (
        echo %LOG_PREFIX% WARNING: SCRFD download failed — face recognition will be unavailable>&2
    ) else (
        echo %LOG_PREFIX% SCRFD model downloaded.>&2
    )
) else (
    echo %LOG_PREFIX% SCRFD model already present.>&2
)

if not exist "%MFNET_FILE%" (
    echo %LOG_PREFIX% Downloading MobileFaceNet ^(5.2 MB^)...>&2
    echo {"event": "progress", "stage": "models", "message": "Downloading MobileFaceNet (5.2 MB)..."}
    "%VPYTHON%" -c "import requests,sys; r=requests.get(sys.argv[1],allow_redirects=True,stream=True); r.raise_for_status(); f=open(sys.argv[2],'wb'); [f.write(c) for c in r.iter_content(8192)]; f.close()" "%MFNET_URL%" "%MFNET_FILE%" 2>&1
    if !errorlevel! neq 0 (
        echo %LOG_PREFIX% WARNING: MobileFaceNet download failed — face recognition will be unavailable>&2
    ) else (
        echo %LOG_PREFIX% MobileFaceNet model downloaded.>&2
    )
) else (
    echo %LOG_PREFIX% MobileFaceNet model already present.>&2
)

echo {"event": "progress", "stage": "models", "message": "Face recognition models ready"}
