...
Communication protocol for Hawkeye detection skills integrated with SharpAI Hawkeye.

## Transport

- **stdin** (Hawkeye → Skill): frame events and commands
- **stdout** (Skill → Hawkeye): detection results, ready/error events
- **stderr**: logging only — ignored by Hawkeye data parser
...
```jsonl
{"event": "frame", "frame_id": 42, "camera_id": "front_door", "timestamp": "2026-03-01T14:30:00Z", "frame_path": "/tmp/hawkeye_detection/frame_front_door.jpg", "width": 1920, "height": 1080}
```

### Detections (Skill → Hawkeye)
...
Indicates a processing error. `retriable: true` means Hawkeye can send the next frame.
...
Frames are written to `/tmp/hawkeye_detection/frame_{camera_id}.jpg` as JPEG files with recycled per-camera filenames (overwritten each cycle). The `frame_path` in the frame event is the absolute path to the JPEG file.
...
The protocol is **request-response**: Hawkeye sends one frame, waits for the detection result, then sends the next. This provides natural backpressure — if the skill is slow, Hawkeye automatically drops frames (always uses the latest available frame).
